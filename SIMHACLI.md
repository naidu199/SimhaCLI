...
Global config: `~/.simhacli/config.toml`
...
Place a `.py` file in `<project>/.simhacli/tools/`:
...
1. **No test suite** — project has no pytest/unittest configuration; test manually via `simhacli`
...
- **Entry point is `main:main`** — defined in `pyproject.toml` as `simhacli = "main:main"`
- **Packages must be listed in `pyproject.toml`** under `[tool.setuptools] packages = [...]`
- **Config uses `platformdirs`** — cross-platform paths via `user_config_dir("simhacli")` and `user_data_dir("simhacli")`
